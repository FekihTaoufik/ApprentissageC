...
int main(int argc, char *argv[])
{
  ...
  r = creation_usine(&usine, &stock_entrepot,&mutex);
  if (r != 0)
    fprintf(stderr, "Usine echec !");

  // Création d'un entrepot
  r = creation_entrepot(&entrepot, &stock_entrepot,&mutex);
  if (r != 0)
  ...
  for (i = 0; i < NB_CONCESSION; i++)
  {
    ...
    *pid = i;
    r = creation_concession(&(concession[i]), pid, &stock_entrepot,&mutex);
    if (r != 0)
